...
setup() ->
  {ok, _StatusPid} = barrel_task_status:start_link(),
  TaskUpdaterPid = spawn(fun() -> loop() end),
  TaskUpdaterPid.


teardown(TaskUpdaterPid) ->
  barrel_lib:shutdown_sync(TaskUpdaterPid),
  barrel_task_status:stop().


barrel_task_status_test_() ->
  {
    "Barrel task status updates",
    {foreach,
...
should_reset_control_update_frequency(Pid) ->
  ?_assertEqual(87,
    begin
      ok = call(Pid, add, [{type, replication}, {progress, 0}]),
...
should_finish_task(Pid) ->
  ok = call(Pid, add, [{type, replication}, {progress, 0}]),
  ?assertEqual(1, length(barrel_task_status:all())),
  ok = call(Pid, done),
...
run_multiple_tasks() ->
  ...
  ?assertEqual(3, length(barrel_task_status:all())),
  ...
  call(Pid1, done),
  ?assertEqual(2, length(barrel_task_status:all())),
  call(Pid3, done),
  ?assertEqual(1, length(barrel_task_status:all())),
  call(Pid2, done),
  ?assertEqual(0, length(barrel_task_status:all())),
...
loop() ->
  receive
    {add, Props, From} ->
      Resp = barrel_task_status:add_task(Props),
      From ! {ok, self(), Resp},
      loop();
    {update, Props, From} ->
      Resp = barrel_task_status:update(Props),
      From ! {ok, self(), Resp},
      loop();
    {update_frequency, Msecs, From} ->
      Resp = barrel_task_status:set_update_frequency(Msecs),
      From ! {ok, self(), Resp},
      loop();
    {done, From} ->
      From ! {ok, self(), ok}
...
get_task_prop(Pid, Prop) ->
  ...
  Element = lists:foldl(
    ...
    [], barrel_task_status:all()
  ),
  case proplists:get_value(Prop, hd(Element), nil) of
    nil ->
      erlang:error({assertion_failed,
        [{module, ?MODULE},
          {line, ?LINE},
          {reason, "Could not get property '"
            ++ barrel_lib:to_list(Prop)
            ++ "' for task "
            ++ pid_to_list(Pid)}]});
    Value ->
      Value
