-module(barrel_acceptor_pool_SUITE).
...
init_per_suite(Config) ->
  {ok, Started} = application:ensure_all_started(barrel),
...
init_per_testcase(_TestCase, Config) ->
  ...
  Spec = #{id => barrel_acceptor_pool_test,
    start => {barrel_acceptor_pool_test, Init, []},
    restart => ?config(restart, Config),
    shutdown => ?config(shutdown, Config),
    grace => ?config(grace, Config)},
  {ok, Pool} = barrel_acceptor_pool_test:start_link(Spec),
  {ok, Ref} = barrel_acceptor_pool:accept_socket(Pool, LSock, 1),
...
which_sockets(Config) ->
  ...
  [{inet_tcp, {_, Port}, LSock, Ref}] = barrel_acceptor_pool:which_sockets(Pool),
...
which_children(Config) ->
  ...
  [] = barrel_acceptor_pool:which_children(Pool),
  ...
  [{{barrel_acceptor_pool_test, {_, _}, {_,_}, _}, Pid, worker,
    [barrel_acceptor_pool_test]}] = barrel_acceptor_pool:which_children(Pool),
...
count_children(Config) ->
  ...
  [{specs, 1}, {active, 0}, {supervisors, 0}, {workers, 1}] =
    barrel_acceptor_pool:count_children(Pool),
  ...
  [{_, Pid, _, _}] = barrel_acceptor_pool:which_children(Pool),
...
format_status(Config) ->
  ...
  {supervisor, [{"Callback", barrel_acceptor_pool_test}]} =
...
start_error(Config) ->
  ...
  Spec = #{id => barrel_acceptor_pool_test,
    start => {barrel_acceptor_pool_test, {error, shutdown}, []},
    restart => ?config(restart, Config)},
  {ok, Pool} = barrel_acceptor_pool_test:start_link(Spec),
  
  {ok, LSock} = gen_tcp:listen(0, []),
  {ok, _} = barrel_acceptor_pool:accept_socket(Pool, LSock, 1),
...
child_error(Config) ->
  ...
  [{_, Pid1, _, _}, {_, Pid2, _, _}] = barrel_acceptor_pool:which_children(Pool),
...
change_spec(_) ->
  Fun = fun() ->
    {ok, Spec} = application:get_env(barrel_acceptor_pool, change_spec),
    Spec
        end,
  
  Spec1 = #{id => barrel_acceptor_pool_test1,
    start => {barrel_acceptor_pool_test, {ok, undefined}, []}},
  ok = application:set_env(barrel_acceptor_pool, change_spec, Spec1),
  {ok, Pool} = barrel_acceptor_pool_test:start_link(Fun),
  ...
  [{{barrel_acceptor_pool_test1, _, _, _}, _, worker, [barrel_acceptor_pool_test]}] =
    barrel_acceptor_pool:which_children(Pool),
  
  Spec2 = #{id => barrel_acceptor_pool_test2,
    start => {barrel_acceptor_pool_test, {ok, trap_exit}, []},
    type => supervisor,
    modules => dynamic},
  ok = application:set_env(barrel_acceptor_pool, change_spec, Spec2),
  
  ok = sys:suspend(Pool),
  ok = sys:change_code(Pool, barrel_acceptor_pool_test, undefined, undefined),
  ok = sys:resume(Pool),
  ...
  [{{barrel_acceptor_pool_test2, _, _, _}, Pid1, supervisor, dynamic},
    {{barrel_acceptor_pool_test2, _, _, _}, Pid2, supervisor, dynamic}] =
    barrel_acceptor_pool:which_children(Pool),
  ...
  [{{barrel_acceptor_pool_test2, _, _, _}, Pid3, supervisor, dynamic},
    {{barrel_acceptor_pool_test2, _, _, _}, Pid4, supervisor, dynamic},
    {{barrel_acceptor_pool_test2, _, _, _}, Pid5, supervisor, dynamic}] =
    barrel_acceptor_pool:which_children(Pool),
...
change_ignore(_) ->
  Fun = fun() ->
    {ok, Spec} = application:get_env(barrel_acceptor_pool,
      change_ignore),
    Spec
        end,
  
  Spec1 = #{id => barrel_acceptor_pool_test,
    start => {barrel_acceptor_pool_test, {ok, undefined}, []},
    modules => dynamic},
  ok = application:set_env(barrel_acceptor_pool, change_ignore, Spec1),
  {ok, Pool} = barrel_acceptor_pool_test:start_link(Fun),
  ...
  [{{barrel_acceptor_pool_test, _, _, _}, _, worker, dynamic}] =
    barrel_acceptor_pool:which_children(Pool),
  
  ok = application:set_env(barrel_acceptor_pool, change_ignore, ignore),
...
change_error(_) ->
  ...
  Spec1 = #{id => barrel_acceptor_pool_test,
    start => {barrel_acceptor_pool_test, {ok, undefined}, []},
    modules => dynamic},
  ok = application:set_env(barrel_acceptor_pool, change_error, Spec1),
  {ok, Pool} = barrel_acceptor_pool_test:start_link(Fun),
  ...
  ok = application:set_env(barrel_acceptor_pool, change_error, bad),
  
  ok = sys:suspend(Pool),
  {error, _} = sys:change_code(Pool, barrel_acceptor_pool_test, undefined,
    undefined),
  ok = sys:resume(Pool),
