...
-include_lib("couch_js_functions.hrl").
...
oauth_auth_callback(#httpd{mochi_req = MochiReq} = Req, CbParams) ->
    ...
    false ->
        barrel_log:debug("OAuth handler: signature verification failed for user `~p`~n"
...
set_user_ctx(Req, Name) ->
    case couch_auth_cache:get_user_creds(Name) of
        nil ->
            barrel_log:debug("OAuth handler: user `~p` credentials not found", [Name]),
            Req;
        User ->
...
serve_oauth(#httpd{mochi_req=MochiReq}=Req, Fun, FailSilently) ->
    ...
    barrel_log:debug("OAuth Params: ~p", [Params]),
...
get_callback_params(ConsumerKey, Params, Url) ->
    ...
    {OauthCreds} ->
        ...
        false ->
            ...
            barrel_log:debug("Got OAuth credentials, for ConsumerKey `~p` and "
...
use_auth_db() ->
    case barrel_config:get("couch_httpd_oauth", "use_users_db", "false") of
    "false" ->
        nil;
    "true" ->
        AuthDb = open_auth_db(),
