...
%% `barrel_acceptor_pool'. There are three callbacks.
%%
...
%%       SockName :: barrel_acceptor_pool:name(),
...
%% socket and `Args' is the argument from the `barrel_acceptor_pool:acceptor_spec/0'
%% in the `barrel_acceptor_pool'. This callback should do any setup required before
...
%% down before the `barrel_acceptor_pool' in the supervisor tree (e.g. after the
%% `barrel_acceptor_pool' in a `rest_for_one' or `one_for_all') and the
%% `barrel_acceptor_pool' defining the `grace' specification.
%%
...
%% restart intensity of the `barrel_acceptor_pool', even with a `restart' of
...
%%      PeerName :: barrel_acceptor_pool:name(),
%%      Sock :: gen_tcp:socket(),
%%      State :: term().
%% '''
...
%% reason is an exit signal from the `barrel_acceptor_pool'.
%%
%% `State' is the state returned by `acceptor_init/3'.
-module(barrel_acceptor).
-behaviour(barrel_acceptor_loop).
...
handle_init({ok, State, Timeout}, Mod, SockMod, LSock, Parent, AckRef) ->
  ...
  barrel_acceptor_loop:accept(LSock, Timeout, Parent, ?MODULE, Data);
