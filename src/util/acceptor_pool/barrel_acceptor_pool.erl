...
%% value is of form:
%% ```
%% {AcceptorMod :: module(), AcceptorArg :: term(), Opts :: [barrel_acceptor:option()]}
%% '''
...
start := {module(), any(), [barrel_acceptor:option()]},
...
start_acceptor(SockRef, Acceptors,
  ...
  case Sockets of
    #{SockRef := {SockMod, SockName, Sock}} ->
      {Pid, AcceptRef} =
        barrel_acceptor:spawn_opt(Mod, SockMod, SockName, Sock, Args, Opts),
