...
handle_call({update, IdxState}, _From, #st{idx=Idx, mod=Mod}=State) ->
    ...
    barrel_log:info("Starting index update for db: ~s idx: ~s", Args),
...
handle_call({restart, IdxState}, _From, #st{idx=Idx, mod=Mod}=State) ->
    Args = [Mod:get(db_name, IdxState), Mod:get(idx_name, IdxState)],
    barrel_log:info("Restarting index update for db: ~s idx: ~s", Args),
...
handle_info({'EXIT', _, {updated, Pid, IdxState}}, #st{pid=Pid}=State) ->
    Mod = State#st.mod,
    Args = [Mod:get(db_name, IdxState), Mod:get(idx_name, IdxState)],
    barrel_log:info("Index update finished for db: ~s idx: ~s", Args),
