...
fixed_case(Bloom, Size, FalseRate) ->
  ?assert(barrel_bloom:capacity(Bloom) > Size),
  ?assertEqual(0, barrel_bloom:size(Bloom)),
  RandomList = simple_shuffle(lists:seq(1,100*Size), Size),
  [?assertEqual(false, barrel_bloom:is_element(E, Bloom)) || E <- RandomList],
  Bloom2 =
...
scalable_case(Bloom, Size, FalseRate) ->
  ?assertEqual(infinity, barrel_bloom:capacity(Bloom)),
  ?assertEqual(0, barrel_bloom:size(Bloom)),
