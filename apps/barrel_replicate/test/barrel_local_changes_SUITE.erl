...
collect_change(_Config) ->
  ...
  Doc = #{ <<"id">> => <<"aa">>, <<"v">> => 1},
  {ok, <<"aa">>, _RevId} = barrel_db:put(<<"testdb">>, Doc, []),
  timer:sleep(100),
...
collect_changes(_Config) ->
  ...
  [] = barrel_local_changes:changes(Pid),
  Doc = #{ <<"id">> => <<"aa">>, <<"v">> => 1},
  {ok, <<"aa">>, _} = barrel_db:put(<<"testdb">>, Doc, []),
  timer:sleep(100),
  [#{ <<"seq">> := 1, <<"id">> := <<"aa">>}] = barrel_local_changes:changes(Pid),
  [] = barrel_local_changes:changes(Pid),
  Doc2 = #{ <<"id">> => <<"bb">>, <<"v">> => 1},
  {ok, <<"bb">>, _} = barrel_db:put(<<"testdb">>, Doc2, []),
  {ok, _} = barrel_db:get(<<"testdb">>, <<"bb">>, []),
  timer:sleep(100),
  [#{ <<"seq">> := 2, <<"id">> := <<"bb">>}] = barrel_local_changes:changes(Pid),
  [] = barrel_local_changes:changes(Pid),
  Doc3 = #{ <<"id">> => <<"cc">>, <<"v">> => 1},
  Doc4 = #{ <<"id">> => <<"dd">>, <<"v">> => 1},
  {ok, <<"cc">>, _} = barrel_db:put(<<"testdb">>, Doc3, []),
  {ok, <<"dd">>, _} = barrel_db:put(<<"testdb">>, Doc4, []),
  {ok, _} = barrel_db:get(<<"testdb">>, <<"cc">>, []),
  {ok, _} = barrel_db:get(<<"testdb">>, <<"dd">>, []),
  timer:sleep(100),
  [
    #{ <<"seq">> := 3, <<"id">> := <<"cc">>},
    #{ <<"seq">> := 4, <<"id">> := <<"dd">>}
...
changes_feed_callback(_Config) ->
  ...
  Doc1 = #{ <<"id">> => <<"aa">>, <<"v">> => 1},
  Doc2 = #{ <<"id">> => <<"bb">>, <<"v">> => 1},
  {ok, <<"aa">>, _} = barrel_db:put(<<"testdb">>, Doc1, []),
  {ok, <<"bb">>, _} = barrel_db:put(<<"testdb">>, Doc2, []),
  {ok, _} = barrel_db:get(<<"testdb">>, <<"aa">>, []),
  {ok, _} = barrel_db:get(<<"testdb">>, <<"bb">>, []),
  timer:sleep(100),
  [
    #{ <<"seq">> := 1, <<"id">> := <<"aa">>},
