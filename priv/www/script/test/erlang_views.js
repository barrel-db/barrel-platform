...
couchTests.erlang_views = function(debug) {
    ...
    run_on_modified_server(
        ...
        function() {
            ...
            var mfun = 'fun(Doc) -> ' +
                ' K = maps:get(<<"integer">>, Doc, null), ' +
                ' V = maps:get(<<"string">>, Doc, null), ' +
                ' Emit(K, V) ' +
                'end.';
            ...
            var designDoc = {
                ...
                shows: {
                    simple:
                    'fun(Doc, Req) -> ' +
                    '  Info = maps:get(<<"info">>, Req, #{}), ' +
                    '  Purged = maps:get(<<"purge_seq">>, Info, -1), ' +
                    '  Verb = maps:get(<<"method">>, Req, <<"not_get">>), ' +
            ...
            var mfun = 'fun(Doc) -> ' +
                'Words = maps:get(<<"words">>, Doc), ' +
                'lists:foreach(fun(Word) -> ' +
                'WordString = maps:get(<<"word">>, Word), ' +
                'Count = maps:get(<<"count">>, Word), ' +
